...
typedef RT_MODEL_q_rotpen_mdl_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * q_rotpen_mdl(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(q_rotpen_mdl_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://q_rotpen_mdl:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: q_rotpen_mdl -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,

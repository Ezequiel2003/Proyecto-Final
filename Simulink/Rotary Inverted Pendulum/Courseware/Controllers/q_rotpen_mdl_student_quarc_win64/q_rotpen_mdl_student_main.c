...
typedef RT_MODEL_q_rotpen_mdl_student_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * q_rotpen_mdl_student(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(q_rotpen_mdl_student_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: q_rotpen_mdl_student -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,

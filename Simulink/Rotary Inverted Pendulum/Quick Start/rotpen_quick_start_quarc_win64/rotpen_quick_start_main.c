...
typedef RT_MODEL_rotpen_quick_start RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * rotpen_quick_start(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(rotpen_quick_start_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(rotpen_quick_start_DWork.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: rotpen_quick_start -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      result = hil_task_start
        (rotpen_quick_start_DWork.HILReadEncoderTimebase_Task, (t_clock)
         rotpen_quick_start_P.HILReadEncoderTimebase_Clock, 500.0, -1)
        ;
      if (result == 0) {

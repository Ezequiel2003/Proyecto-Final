...
com_serial = serial.Serial(COM,115200,8,stopbits=1)
...
def encontrar_puntob(img2,flag):
    ...
    desplazamiento_mask = -1*cv2.getTrackbarPos('Copia Desplazada', 'Mask')
    ...
    if not(flag): #Cuando no está calibrado
        ...
        _,binarizado = cv2.threshold(img,50,255,cv2.THRESH_BINARY)
    ...
    #cv2.imshow()
        ...
        if(seg):
            ...
            y1 = p_blanco[1] - int(tolerancia*1)
            y2 = p_blanco[1] + int(tolerancia*1.5)
        else:
            tolerancia = 10
...
cv2.createTrackbar('X_ini', 'Mask', 293, ancho, nada)
cv2.createTrackbar('X_final', 'Mask', 315, ancho, nada)#252
cv2.createTrackbar('Y_ini', 'Mask', 369, alto, nada)#38
cv2.createTrackbar('Y_final', 'Mask', 385, alto, nada)#60
cv2.createTrackbar('Copia Desplazada', 'Mask', 351, alto, nada)#128
...
while(1):
  ...
  if(ret): #Si no hay fallo en capturar el frame
      ...
      if not calib :
        ...
        p_ref,p_ini,seg = encontrar_puntob(frame,calib) #Comentado únicamente para imágenes estáticas
...
#          #print("Centro de rotacion: ",centro)
          ...
          if(seg):cv2.line(frame, p_ref, p_blanco, (0,255,255),2)
          ...
          #cv2.putText(frame, "Angulo:"+str(theta)+"Rad", position2, font, fontScale, fontColor,thick) #Comentado para sacar capturas
      ...
      cv2.putText(frame, "FPS:"+str(fps), position3, font, fontScale, fontColor,thick)      #comentado para sacar capturas
      ...
      if k == ord("q"):
        com_serial.write(str(0).encode() + b'\r\n')

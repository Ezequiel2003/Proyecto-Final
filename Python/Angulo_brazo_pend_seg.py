...
cap_brazo = cv2.VideoCapture(1, cv2.CAP_DSHOW)
...
cap_pend = cv2.VideoCapture(2, cv2.CAP_DSHOW)
...
def encontrar_puntob(img2,p_ini,flag_cal,tipo): #frame,flag calibración y el tipo, o sea si es brazo o péndulo
    ...
    En el modo de calibración, se ubican máscaras seleccionadas desde barras deslizantes para ubicar la posición de las mismas.
    ...
    if not(flag_cal): #Cuando no está calibrado
        ...
        if(not (tipo)): #Si se trata del brazo
            ...
            mask_brazo.guardar((x_ini_mask,x_fin_mask),(a,b)) #Guarda en la instancia "mask_brazo" las posiciones de las máscaras
            ...
            desplazamiento_mask = -1*cv2.getTrackbarPos('Copia Desplazada', 'Mask_pend')
            a = y_fin_mask+desplazamiento_mask
            b = (2*y_fin_mask) + desplazamiento_mask - y_ini_mask
            mask_pend.guardar((x_ini_mask,x_fin_mask),(a,b)) #Guarda en la instancia "mask_pend" las posiciones de las máscaras
...
cv2.createTrackbar('Copia Desplazada', 'Mask_pend', 338, alto, nada)#128

...
def encontrar_puntob(img2,flag):
    ...
    if not(flag): #Cuando no está calibrado
        ...
        _,binarizado = cv2.threshold(img,127,255,cv2.THRESH_BINARY)
    ...
    #cv2.imshow()
        ...
        #binarización OTSU
        #_,binarizado = cv2.threshold(img, 0,255, cv2.THRESH_BINARY+cv2.THRESH_OTSU)
        _,binarizado = cv2.threshold(img, 180,255, cv2.THRESH_BINARY)
        ...
        if(seg):
            ...
            y1 = p_blanco[1] - int(tolerancia*1.5)
...
cv2.createTrackbar('X_ini', 'Mask', 224, ancho, nada)
cv2.createTrackbar('X_final', 'Mask', 240, ancho, nada)#252
cv2.createTrackbar('Y_ini', 'Mask', 269, alto, nada)#38
cv2.createTrackbar('Y_final', 'Mask', 284, alto, nada)#60
cv2.createTrackbar('Copia Desplazada', 'Mask', 110, alto, nada)#128

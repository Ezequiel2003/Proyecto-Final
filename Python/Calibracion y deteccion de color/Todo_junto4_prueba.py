...
def encontrar_puntob(img2,flag):
    global p_anterior,x1,x2,y1,y2 #guardo la última vez en donde estuvo el punto blanco
...
cv2.createTrackbar('Y_ini', 'Mask', 38, alto, nada)
cv2.createTrackbar('Y_final', 'Mask', 60, alto, nada)
cv2.createTrackbar('Copia Desplazada', 'Mask', 128, alto, nada)
...
while(1):
  ...
  if(ret): #Si no hay fallo en capturar el frame
      ...
      theta = 0
      if not calib :
        f_anterior = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        p_ref,p_ini = encontrar_puntob(frame,calib)
...
#          frame = cv2.warpAffine(src=frame2, M = matriz_rot,dsize=(frame2.shape[1],frame2.shape[0]))
          ...
          _,p_blanco = encontrar_puntob(frame,calib)
